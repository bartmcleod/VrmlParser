			...
			switch ( node.node ) {
				...
				case 'IndexedFaceSet':
					...
					if ( node.has('coordIndex') ) {
						...
						for ( var i = 0, j = node.coordIndex.length; i < j; i ++ ) {
							...
							while ( indexes.length >= 3 && skip < ( indexes.length - 2 ) ) {
								...
								// create entries in the buffer
								function _pushToFaceBuffer(index1, index2, face) {
					...
					for ( var a in facesBuffer ) {
						...
						var face2 = facesBuffer[ a ].faces[ 1 ];
						...
						facesBuffer[ a ].edge = angle > creaseAngle ? 'sharp' : 'smooth';

						scope.log(face1.normal);
						scope.log(face2.normal);
